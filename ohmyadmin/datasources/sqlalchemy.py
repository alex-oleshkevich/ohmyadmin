...
def get_column_properties(
    model_class: typing.Any, prop_names: typing.Sequence[str]
) -> dict[str, orm.ColumnProperty]:
    """
...
def guess_pk_field(model_class: type) -> str:
    ...
    assert len(
        pk_columns
    ), f"Model class {model_class} does not contain any primary key."
    assert (
        len(pk_columns) == 1
    ), f"Model class {model_class} defines composite primary key which are not supported."
    return pk_columns[0]
...
def guess_pk_type(model_class: type, pk_field: str) -> typing.Callable:
    ...
    match column.type:
        ...
        case sa.Uuid():
            return uuid.UUID
    raise ValueError(
        f"Failed to guess primary key column caster for {column} (type={column.type})."
    )
...
class SADataSource(DataSource[T]):
    def __init__(
        ...
        self.query_for_list = (
            query_for_list if query_for_list is not None else self.query
        )
    ...
    def apply_search_filter(
        ...
        for prop in props.values():
            ...
            match predicate:
                case "startswith":
                    clauses.append(
                        sa.func.lower(string_column).startswith(term.lower())
                    )
                case "endswith":
                    clauses.append(sa.func.lower(string_column).endswith(term.lower()))
                case "matches":
                    clauses.append(
                        sa.func.lower(string_column).regexp_match(term.lower())
                    )
                case "exact":
    ...
    def apply_ordering_filter(
        ...
        for ordering_field, ordering_dir in rules.items():
            try:
                ...
                column = prop.columns[0]
                stmt = stmt.order_by(
                    column.desc() if ordering_dir == "desc" else column.asc()
                )
            except KeyError:
