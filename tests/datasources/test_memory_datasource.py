...
class Post:
    ...
    date_published: datetime.date | None = dataclasses.field(
...
async def test_create(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
    post = Post(id=100)
...
async def test_search_starts_with(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
    qs = datasource.apply_search_filter("^Title 2", searchable_fields=["title"])
    obj = await qs.paginate(http_request, 1, 20)
...
async def test_search_ends_with(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
...
async def test_not_sorts_unsupported_fields(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
...
async def test_string_filter_startswith(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
...
async def test_string_filter_endswith(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
...
async def test_string_filter_contains(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
...
async def test_string_filter_pattern(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
    qs = datasource.apply_string_filter(
        "title", StringOperation.pattern, r"Title 1([12])"
    )
    obj = await qs.paginate(http_request, 1, 20)
    assert len(obj.rows) == 2
...
async def test_string_filter_exact(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
...
async def test_number_filter_equals(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
...
async def test_number_filter_gt(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
...
async def test_number_filter_gte(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
...
async def test_number_filter_lt(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
...
async def test_number_filter_lte(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
...
async def test_datetime_filter(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
    qs = datasource.apply_date_filter(
...
async def test_date_range_filter_before(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
...
async def test_date_range_filter_null_after(http_request: Request) -> None:
    datasource = InMemoryDataSource(Post, [Post(date_published=None)])
    qs = datasource.apply_date_range_filter(
        "date_published", after=datetime.date(2023, 1, 2), before=None
    )
    obj = await qs.paginate(http_request, 1, 20)
    assert [obj.date_published for obj in obj] == []


async def test_date_range_filter_after(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
    qs = datasource.apply_date_range_filter(
...
async def test_date_range_filter_between(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
    qs = datasource.apply_date_range_filter(
        "date_published",
...
async def test_datetime_range_filter_before(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
    qs = datasource.apply_date_range_filter(
...
async def test_datetime_range_filter_after(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
    qs = datasource.apply_date_range_filter(
        "updated_at",
...
async def test_datetime_range_filter_between(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
    qs = datasource.apply_date_range_filter(
        "updated_at",
...
async def test_choice_filter_mismatch(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
...
async def test_choice_filter_coerce(
    datasource: InMemoryDataSource[Post], http_request: Request
) -> None:
...
async def test_boolean_filter_true(http_request: Request) -> None:
    datasource = InMemoryDataSource(
        Post, [Post(id=1, published=True), Post(id=2, published=False)]
    )
