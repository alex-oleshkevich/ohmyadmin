...
from ohmyadmin.authentication import (
    SESSION_KEY,
    AnonymousAuthPolicy,
    AnonymousUser,
    BaseAuthPolicy,
...
def test_pages_not_accessible_for_unauthenticated(
    ...
    assert (
        ...
        == "http://testserver/admin/login?next=/admin/example"
    )


def test_welcome_not_accessible_for_unauthenticated(
    create_test_app: CreateTestAppFactory,
) -> None:
...
def test_media_not_accessible_for_unauthenticated(
    create_test_app: CreateTestAppFactory,
) -> None:
    ...
    assert (
        response.headers["location"]
        == "http://testserver/admin/login?next=/admin/media/file.txt"
    )


def test_login_is_accessible_for_unauthenticated(
    create_test_app: CreateTestAppFactory,
) -> None:
...
def test_successful_login_logout(create_test_app: CreateTestAppFactory) -> None:
    ...
    response = client.post(
        "/admin/login",
        data={"identity": "valid@localhost.tld", "password": "pass"},
...
def test_failed_login_logout(create_test_app: CreateTestAppFactory) -> None:
    ...
    response = client.post(
        "/admin/login",
        data={"identity": "invalid@localhost.tld", "password": "pass"},
        allow_redirects=False,
    )
...
async def test_login_required_middleware_checks_request_types() -> None:
    ...
    await app(
        {"type": "unknown", "headers": [], "path": "/", "router": mock.MagicMock()},
        receive,
        send,
    )
    ...
    await app(
        {"type": "http", "headers": [], "path": "/", "router": mock.MagicMock()},
        receive,
        send,
    )
    ...
    await app(
        {"type": "websocket", "headers": [], "path": "/", "router": mock.MagicMock()},
        receive,
        send,
    )
    child_app.assert_not_awaited()  # unauthenticated
