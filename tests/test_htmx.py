...
def test_is_htmx_request(request_f: RequestFactory) -> None:
    request = request_f(headers=[(b"hx-request", b"yes")])
    htmx.is_htmx_request(request)

    request = request_f(headers=[])
    assert not htmx.is_htmx_request(request)


def test_matches_target(request_f: RequestFactory) -> None:
    request = request_f(headers=[(b"hx-target", b"main")])
