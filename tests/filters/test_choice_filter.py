...
async def test_renders_form(request_f: RequestFactory) -> None:
    unbound = ChoiceFilter('example', choices=[])
    ...
    assert page.has_node('select[name="example-choice"]')
...
async def test_renders_form_with_async_choices(request_f: RequestFactory) -> None:
    ...
    unbound = ChoiceFilter('example', choices=async_choices)
    assert isinstance(unbound, UnboundFilter)
...
async def test_is_active(request_f: RequestFactory) -> None:
    unbound = ChoiceFilter('example', choices=[('1', 'One')])
    assert isinstance(unbound, UnboundFilter)
...
async def test_applies_coercion(request_f: RequestFactory, datasource: InMemoryDataSource[Post]) -> None:
    unbound = ChoiceFilter('example', choices=[('1', 'One')], coerce=int)
    assert isinstance(unbound, UnboundFilter)
