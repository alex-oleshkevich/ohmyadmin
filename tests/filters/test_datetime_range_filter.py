...
async def test_is_active(
    request_f: RequestFactory, query_string: str, expectation: bool
) -> None:
    unbound = DateTimeRangeFilter("example")
    assert isinstance(unbound, UnboundFilter)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "example-before=2023-01-02 00:00:00",
            ("example", datetime.datetime(2023, 1, 2, 0, 0, 0), None),
        ),
        (
            "example-after=2023-01-02 00:00:00",
            ("example", None, datetime.datetime(2023, 1, 2, 0, 0, 0)),
        ),
        (
...
async def test_not_applies_if_not_active(
...
@pytest.mark.parametrize(
    ...
    [
        "example-before=2023",
        "example-after=2023",
        "example-after=2023&example-before=2023",
    ],
)
...
async def test_renders_indicator(request_f: RequestFactory) -> None:
    ...
    request = request_f(
        query_string="example-after=2023-01-02 00:00:00&example-before=2023-01-02 00:00:00"
    )
    instance = await unbound.create(request)
    content = instance.render_indicator(request)
    page = MarkupSelector(content)
