...
def test_page() -> None:
    ...
    page = Pagination(rows, total_rows=101, page=2, page_size=10)
...
def test_page_iterator() -> None:
    rows = [1, 2]
    page = Pagination(rows, total_rows=2, page=1, page_size=2)
...
def test_page_no_other_pages() -> None:
    page: Pagination = Pagination([], total_rows=2, page=1, page_size=2)
...
def test_iterator_without_pages() -> None:
    page: Pagination = Pagination([], total_rows=0, page=1, page_size=1)
    with pytest.raises(StopIteration):
...
def test_page_next_prev_pages() -> None:
    rows = [1, 2]
    page = Pagination(rows, total_rows=1, page=1, page_size=1)
