...
class MyPage(FormPage):  # pragma: no cover
    ...
    async def handle_submit(
        self, request: Request, form: MyForm, model: typing.Any
    ) -> Response:
...
async def test_renders_default_form_action(
...
async def test_renders_custom_form_actions(
    create_test_app: CreateTestAppFactory,
) -> None:
...
async def test_populates_form_object(create_test_app: CreateTestAppFactory) -> None:
    ...
    selector = MarkupSelector(response.text)
    assert (
        selector.get_attribute('form input[type="text"][name="name"]', "value")
        == "TEST"
    )
