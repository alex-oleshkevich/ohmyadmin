...
@pytest.mark.parametrize('value', [datetime.datetime(2023, 1, 2, 12, 30, 59), datetime.time(12, 30, 59)])
def test_time_formatter_default_format(http_request: Request, value: datetime.time | datetime.datetime) -> None:
...
@pytest.mark.parametrize('value', [datetime.datetime(2023, 1, 2, 12, 30, 59), datetime.time(12, 30, 59)])
def test_time_formatter_short_format(http_request: Request, value: datetime.time | datetime.datetime) -> None:
    formatter = TimeFormatter(format='short')
    content = formatter.format(http_request, value)
    selector = MarkupSelector(content)
    assert selector.get_text('time') == '12:30 PM'
...
@pytest.mark.parametrize('value', [datetime.datetime(2023, 1, 2, 12, 30, 59), datetime.time(12, 30, 59)])
def test_time_formatter_medium_format(http_request: Request, value: datetime.time | datetime.datetime) -> None:
...
@pytest.mark.parametrize('value', [datetime.datetime(2023, 1, 2, 12, 30, 59), datetime.time(12, 30, 59)])
def test_time_formatter_full_format(http_request: Request, value: datetime.time | datetime.datetime) -> None:
