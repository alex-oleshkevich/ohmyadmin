...
@pytest.mark.parametrize(
    "value", [datetime.datetime(2023, 1, 2, 12, 30, 59), datetime.date(2023, 1, 2)]
)
def test_date_formatter_short_format(
    http_request: Request, value: datetime.date | datetime.datetime
) -> None:
    formatter = DateFormatter(format="short")
...
def test_date_formatter_medium_format(
    http_request: Request, value: datetime.date | datetime.datetime
) -> None:
...
def test_date_formatter_full_format(
    http_request: Request, value: datetime.date | datetime.datetime
) -> None:
