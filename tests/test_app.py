    ...
    class ExamplePage(Page):
        async def get(self, request: Request) -> Response:
            return PlainTextResponse(
                request.state.admin.url_for(request, "ohmyadmin.login")
            )
...
def test_static_url(create_test_app: CreateTestAppFactory) -> None:
    class ExamplePage(Page):
        async def get(self, request: Request) -> Response:
            return PlainTextResponse(
                request.state.admin.static_url(request, "js/asset.js")
            )
    ...
    assert client.get("/admin/example").text.startswith(
        "http://testserver/admin/static/js/asset.js"
    )
...
def test_media_url_for_upload(create_test_app: CreateTestAppFactory) -> None:
    class ExamplePage(Page):
        async def get(self, request: Request) -> Response:
            return PlainTextResponse(
                request.state.admin.media_url(request, "uploads/file.txt")
            )
    ...
    # we have to check if the url starts with the expected one
    assert (
        client.get("/admin/example").text
        == "http://testserver/admin/media/uploads/file.txt"
    )
...
def test_media_url_for_http_url(create_test_app: CreateTestAppFactory) -> None:
    ...
    class ExamplePage(Page):
        async def get(self, request: Request) -> Response:
            return PlainTextResponse(
                request.state.admin.media_url(request, "http://example.com/image.gif")
            )
...
def test_media_url_for_https_url(create_test_app: CreateTestAppFactory) -> None:
    ...
    class ExamplePage(Page):
        async def get(self, request: Request) -> Response:
            return PlainTextResponse(
                request.state.admin.media_url(request, "https://example.com/image.gif")
            )
