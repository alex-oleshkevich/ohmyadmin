...
async def test_confirmation(request_f: RequestFactory) -> None:
    ...
    assert selector.get_text('form [data-test="confirmation"]') == ('Are you sure you want to delete selected objects?')


async def test_dispatch(request_f: RequestFactory, datasource: DataSource) -> None:
    request = request_f(method='POST', query_string='_ids=1', state={'datasource': datasource})
    action = BatchDelete()
