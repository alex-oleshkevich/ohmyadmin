...
async def form_modal_handler(request: Request, form: wtforms.Form) -> ActionResponse:
    return ActionResponse().show_toast(json.dumps(form.data))


class _ExampleForm(wtforms.Form):
...
class _DemoPageActions(TablePage):
    ...
    page_actions = [
        ...
        actions.Callback(
            'form',
            label='Form modal',
            callback=actions.FormModal('Modal title', callback=form_modal_handler, form_class=_ExampleForm),
        ),
    ]
...
def test_dispatches_modal_action() -> None:
    ...
    assert json.loads(response.headers['hx-trigger']) == {
        'toast': {'message': '{"first_name": "root", "last_name": "localhost"}', 'category': 'success'}
    }
