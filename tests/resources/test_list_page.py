...
def test_paginates(client: TestClient) -> None:
    ...
    assert response.status_code == 200
    assert selector.has_node('[data-test="datatable"]')
    assert selector.get_text('[data-test="datatable"] tbody tr:nth-child(1) td:nth-child(2)') == 'Title 26'
...
def test_sorts_dataset(client: TestClient) -> None:
    ...
    selector = MarkupSelector(response.text)
    assert response.status_code == 200
    assert selector.get_text('[data-test="datatable"] tbody tr:first-child td:nth-child(2)') == 'Title 99'
...
def test_filters_dataset(client: TestClient) -> None:
    response = client.get('/admin/resources/demo?title-query=Title%2099&title-operation=exact')
    selector = MarkupSelector(response.text)
    assert response.status_code == 200
