...
def test_renders_edit_form(client: TestClient) -> None:
    ...
    assert (
        selector.get_attribute('form input[type="text"][name="title"]', "value")
        == "Title 90"
    )
...
def test_submits_form_and_continue_edit(client: TestClient) -> None:
    response = client.post(
        "/admin/resources/demo/90/edit", data={"title": "new title", "_continue": ""}
    )
...
def test_submits_form_and_returns(client: TestClient) -> None:
    response = client.post(
        "/admin/resources/demo/90/edit", data={"title": "new title", "_return": ""}
    )
    assert response.status_code == 204
