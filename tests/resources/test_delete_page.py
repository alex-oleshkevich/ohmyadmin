...
def test_renders_delete_page(client: TestClient) -> None:
    ...
    assert selector.has_node('form a[href="http://testserver/admin/resources/demo/"].btn-text')
...
def test_dispatches_htmx(client: TestClient, method: str) -> None:
    callback = getattr(client, method)
    response = callback('/admin/resources/demo/1/delete', allow_redirects=False, headers={'hx-request': 'true'})
